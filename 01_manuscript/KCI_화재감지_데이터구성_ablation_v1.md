...
> 작성 상태: v1.5 (2026-06-24). v1.4에서 C2 전체 지표 확정(P=0.586, R=0.441, mAP@0.5:0.95=0.231) 반영, 그림 파일명 규칙 통일(fig_model_dfire.png), §4.2·§5.2 "175K+NM" 표현 정정, §5.4 AIHub 기준 명시, §6.1·§6.2 D-Fire 기준 수치 수정, §6.3 AIHub C2<C1 테스트셋 불일치 주의 추가, 부록 A D-Fire R8 기준 전면 교체. 수치 SSOT = `G:\EXPERT_KCI_Paper\02_data_ssot\TRAINING_LOG.md` (R8 섹션).
...
| 모델 | 파라미터 규모 | mAP@0.5 | 정밀도 | 재현율 | 데이터셋 | 비고 |
|------|--------------|---------|--------|--------|---------|------|
| YOLO11n | 최소 | 0.736 | 0.746 | 0.669 | D-Fire | 엣지 기준 |
| YOLO11s | 소 | **0.749** | 0.767 | 0.672 | D-Fire | +1.3%p |
...
- **C3→C4 구성 비교의 교란 변수**: C4는 C3 대비 NM 35,000장 추가와 동시에 총 학습 데이터 량이 137,520장에서 175,000장으로 증가하였다. 따라서 C3→C4의 성능 향상(+7.5%p)은 NM 포함 효과와 총 데이터 량 증가 효과가 완전히 분리되지 않는다. NM 비율을 동일하게 유지하면서 총량만 통제한 추가 실험은 향후 과제이다.
...
| C4_11s mAP@0.5 / P / R | 0.749 / 0.767 / 0.672 | R8-C4_11s |
