...
> 작성 상태: v1.15 (2026-06-25). v1.15에서 초록 해상도 문구 정합 — 국문·영문 초록 "손실 미미" → "손실 없음, 오히려 소폭 향상(+0.4%p)"으로 §5.4 실험 결과와 일치시킴. 수치 SSOT = `G:\EXPERT_KCI_Paper\02_data_ssot\TRAINING_LOG.md` (R8·R9 섹션).
...
1. **씬 경계 탐지**: 히스토그램 상관계수(cv2.compareHist)가 0.7 이하로 떨어지는 지점을 씬 경계로 판정한다. 임계값 0.7은 예비 실험에서 씬 경계 누락과 과분할 사이의 균형을 최적화하는 값으로 선정하였다.
2. **씬 내 시간 간격 추출**: 씬 내에서 2초 간격(30fps 기준 60프레임당 1장)으로 프레임을 추출한다. 이 간격은 동일 씬 내 시각적 유사 프레임의 중복을 최소화하면서 장면 다양성을 확보하기에 충분한 것으로 예비 실험에서 확인하였다.
3. **지각 해시 중복 제거**: imagehash 기반 perceptual hash의 해밍 거리가 8 이하인 프레임을 중복으로 제거한다. 해밍 거리 임계값 8은 지각적으로 유사한 프레임을 효과적으로 제거하면서 미세한 장면 변화는 보존하는 값으로, imagehash 라이브러리 권장 범위(4~10) 내에서 선정하였다.
...
C2의 수량은 FL:SM 비율(약 14:1)을 통제 변수로 유지하면서 학습셋 내에서 결정된 근사치이다. C4의 NM 수량(6,458장)은 D-Fire 학습셋 내 정상 배경 이미지 전량을 사용한 결과이며, 이로 인해 FL:SM:NM 비율은 약 1:1:2.8이 된다. 정상 배경을 화재·연기보다 많게 구성한 것은 실제 감시 환경에서 화재 발생이 드물고 정상 장면이 대부분을 차지하는 현실을 반영한다. 평가용 검증셋·테스트셋은 전 구성에서 동일한 D-Fire 공식 분할(각 2,997장)을 사용하여 직접 비교가 가능하도록 통제한다.
...
표준 객체 탐지 지표인 mAP@0.5, mAP@0.5:0.95, 정밀도(precision), 재현율(recall), 클래스별 AP를 사용한다.
...
AIHub C4 데이터 기준으로, YOLO11n에서 입력 해상도를 640에서 416으로 낮춘 결과, mAP는 0.911에서 약 0.915로 오히려 소폭 향상(+0.4%p)되었다. 640px 대비 416px에서 정확도 손실이 없을 뿐 아니라 소폭 향상된 것은, 해상도 축소가 일종의 정규화 효과로 작용하거나 본 데이터셋의 화재·연기 객체 크기가 저해상도 입력에서도 충분히 탐지 가능한 규모임을 시사한다. 해상도를 낮추면 엣지에서 처리량이 약 2배로 향상되므로, 정확도 이득을 유지하면서 속도를 확보할 수 있는 자원제약 디바이스에서 유효한 전략이다.
...
- 본 실험은 D-Fire 단일 벤치마크 기준이며, 다른 공개 데이터셋에서의 추가 교차 검증은 향후 과제이다. AIHub 대규모 보강 실험(§5.5)은 동일한 경향을 지지하나, D-Fire는 공식 test split(2,997장), AIHub는 검증셋(val, 19,080장)을 평가셋으로 사용하여 두 실험의 mAP 수치를 직접 비교하는 것은 유효하지 않다.
- **C2 결과의 데이터셋 의존성**: D-Fire에서 C2(0.455)는 C1(0.325)보다 높으나, AIHub 기존 실험(R2/R3)에서는 평가셋 불일치로 인해 C2(0.761) < C1(0.777) 역전 경향이 관찰된 바 있다. 이를 검증하기 위해 동일 검증셋(val, 19,080장) 기준으로 AIHub C1·C2를 재실험한 결과(R9), C2(mAP 0.770) > C1(mAP 0.463)로 D-Fire와 동일한 방향이 확인되었으며 smoke AP도 C1(0.000) → C2(0.616)로 활성화되었다. 기존 R2/R3의 역전 경향은 평가셋 불일치에 기인한 것으로 판단된다. 다만 fire AP는 C1(0.927)이 C2(0.925)보다 미세하게 높아(−0.2%p), 소량의 smoke 학습이 fire 탐지에 미치는 영향은 데이터셋에 따라 다를 수 있음을 시사한다. 이 경향이 확인된다면 "소량 불균형 smoke 추가의 효과"는 데이터셋에 따라 부호가 달라질 수 있음을 시사한다. 그러나 smoke AP 기준의 비선형 패턴(부분 활성 → 완전 부활)과 클래스 균형이 임계점이라는 결론은 두 데이터셋에서 일관된다.
...
**그림 6b. 탐지 실패 사례 (C4 YOLO11n, D-Fire 테스트셋).** 좌: 정상 산악 배경을 smoke로 오탐(FP, 신뢰도 0.25). 우: 이미지 대부분을 채운 짙은 연기를 미탐(FN, 빨간 박스는 GT).
