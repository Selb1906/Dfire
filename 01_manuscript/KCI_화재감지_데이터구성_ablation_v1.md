...
> 작성 상태: v1.7 (2026-06-25). v1.6에서 §5.7 선행 연구 비교(표 5) 신설 — D-Fire 직접 비교(●)/참고 비교(○) 2층위 구조, TBD 수치는 deep-research 결과 대기 중. §6.3 C3→C4 교란 변수 서술에서 AIHub/D-Fire 수치 혼용 수정. 수치 SSOT = `G:\EXPERT_KCI_Paper\02_data_ssot\TRAINING_LOG.md` (R8 섹션).
...
| 연구 | 모델 | 테스트셋 | mAP@0.5 | P | R | 비교 |
|------|------|---------|---------|---|---|------|
| de Venâncio et al. (2022) [6] | 경량 CNN | D-Fire | — †1 | — | — | ● 직접 |
| (D-Fire 기반 후속 연구) | — | D-Fire | TBD | — | — | ● 직접 |
| Wu et al. (2022) [7] | YOLOv5+DC | 자체 데이터 | TBD | TBD | TBD | ○ 참고 |
| Mukhiddinov et al. (2022) [8] | YOLOv5 | 자체(UAV) | TBD | TBD | TBD | ○ 참고 |
| Kim et al. (2024) [9] | YOLOv8n | 자체 데이터 | TBD | TBD | TBD | ○ 참고 |
| Lu et al. (2025) [14] | FCMI-YOLO | 자체 데이터 | TBD | TBD | TBD | ○ 참고 |
| **본 연구 C4 — YOLO11n** | **YOLO11n** | **D-Fire** | **0.736** | **0.746** | **0.669** | **● 직접** |
| **본 연구 C4 — YOLO11s** | **YOLO11s** | **D-Fire** | **0.749** | **0.767** | **0.672** | **● 직접** |

> ● 직접: D-Fire 공식 test split 기준, 직접 수치 비교 유효.
> ○ 참고: 테스트셋 상이, 수치 직접 비교 불가, 방법론적 맥락 참고용.
> †1 de Venâncio et al. (2022)는 mAP 대신 precision·recall·F1 등을 보고. 해당 지표로 대체 기재 예정.

참고용 연구들(○)은 데이터셋이 달라 절대 수치를 직접 비교할 수 없으나, 유사한 자원제약 환경에서의 YOLO 계열 화재 탐지 성능 수준을 보여준다. 직접 비교 대상(●)인 D-Fire 기준 결과만을 선행 연구 대비 성능 평가의 기준으로 삼는다.
