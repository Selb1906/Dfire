...
> 작성 상태: v1.20 (2026-06-25). v1.20: §2.1 선행연구 데이터 구성 미공개 문장 추가; §5.7 "학습 조건 상이(§2.1)" 참조 추가; §5.4+초록 D-Fire @416 −3.6%p 반영(데이터셋 의존적으로 수정); §6.3 Option B 결과 반영(NM 고유 효과 확인, NM 포화 관찰); §5.3 Option B 교차 참조 추가; §5.2 YOLO11l 발산 설명 보강; §6.3 여섯째 한계(단일 모델 계열) 추가. v1.19에서 D-Fire 데이터셋 분할 수 전면 정정 — 원고의 train 15,533/val 2,997/test 2,997 → 실제 사용 배포판 기준 train 14,122/val 3,099/test 4,306으로 수정(§3.1·3.2·3.3·5.7·6.3). R8 수치는 전부 4,306 기준으로 정확히 산출된 것으로 확인, 수치 변경 없음. v1.18에서 §1.1 비화재보 통계 실수치 반영 — "상당 부분" → "2017→2022년 약 5배 급증(소방청 보도자료[18])". 참고문헌 [18] 추가. ⚠️ [18] 투고 전 URL 확인 필수. 수치 SSOT = `G:\EXPERT_KCI_Paper\02_data_ssot\TRAINING_LOG.md` (R8·R9 섹션).
...
자동화재탐지설비는 건축물 방재의 1차 방어선이다. 그러나 감지기 단독 경보는 조리 연기, 수증기, 분진, 담배 연기, 감지기 노후·오염 등 화재가 아닌 요인에 의해 빈번히 작동한다. 소방청 빅데이터 분석에 따르면 2017년 대비 2022년 비화재보 발생건수는 약 5배 급증하였으며[18], 이는 비화재보 문제가 국내 건물 화재 대응 체계에서 지속적으로 심화되고 있음을 보여준다. 이러한 비화재보는 거주자의 경보 무시(alarm fatigue)를 유발하고, 소방 출동 자원을 비효율적으로 소모하며, 결과적으로 실제 화재 시 초기 대응을 지연시키는 안전상의 역설을 낳는다.
...
본 연구는 세 가지 연구 질문을 설정한다. 첫째, 화재·연기 탐지 성능을 결정하는 주요인은 학습 데이터 구성인가, 모델 용량인가? 둘째, 비화재보 저감에 직접 기여하는 데이터 요소는 무엇인가? 셋째, 엣지 배포 시 입력 해상도를 낮추어 얻는 속도 이득과 정확도 손실의 균형점은 어디인가?

### 1.3 기여

본 연구의 기여는 세 가지이다. 첫째, 동일한 탐지기 계열에서 데이터 구성과 모델 용량을 분리한 통제된 ablation 실험을 수행하여 두 요인의 기여도를 정량 비교한다(데이터 구성 효과 약 30배 우세). 둘째, 화염·연기 클래스 균형과 정상배경 포함이 각각 비화재보 저감에 기여하는 메커니즘을 실측 정밀도·재현율 변화로 제시한다. 셋째, 엣지 디바이스(Jetson Orin Nano) 배포를 전제로 입력 해상도-정확도 트레이드오프를 측정하여 실무 배포 지침을 제시한다.
...
객체 탐지 분야에서는 배경(negative) 표본의 품질이 오탐 억제에 결정적임이 알려져 있다. 특히 검출기가 오경보를 일으키는 어려운 음성 표본(hard negative)을 학습에 반복 투입하는 hard negative mining이 정밀도 향상에 효과적임이 보고되었다[12]. 화재 탐지에 이를 적용하면, 화재 유사 패턴(고온 조명, 연무, 석양 등)을 포함한 정상 장면을 배경 음성 표본(background negatives)으로 명시적으로 학습에 포함함으로써 비화재보 저감에 직접 기여할 수 있다. 본 연구에서는 오탐 기반 동적 선별 없이 학습셋 내 정상 배경 이미지 전량을 정적으로 포함하는 방식을 채택하며, 이를 이하 정상배경(NM) 표본으로 지칭한다.
...
**본 실험(ablation 주실험)**에는 D-Fire 공개 데이터셋[6]을 사용한다. D-Fire는 de Venâncio et al. (2022)이 공개한 화재·연기 탐지 전용 이미지 데이터셋으로, 총 21,527장이 YOLO 형식 바운딩박스 주석과 함께 제공된다. 본 연구에서 사용한 공개 배포판(sayedgamal99/smoke-fire-detection-yolo) 기준 분할은 학습 14,122장 / 검증 3,099장 / 테스트 4,306장이며, 배포판에 따라 분할 수가 상이할 수 있다. 화염(fire)·연기(smoke) 2클래스와 정상 배경(라벨 없음) 이미지를 포함하며, 단일 공개 벤치마크로 평가셋을 고정할 수 있어 실험 간 직접 비교가 가능하다는 장점이 있다. 모든 라벨은 fire(클래스 0)·smoke(클래스 1)의 2클래스로 통합하며, 정상 장면은 라벨이 없는 빈 주석 파일로 처리하여 배경 음성 표본(NM)으로 활용한다.

**대규모 재현 실험(§5.6)**에는 AIHub 화재 발생 예측 영상 데이터(과제번호 071751)[10]를 추가로 활용한다. 본 데이터는 총 약 175,000장 규모의 국내 화재 영상으로, train/val 2분할만 제공되어 별도 test 분할이 없다. 검증셋(val, 약 19,080장)을 평가셋으로 고정하여 모든 AIHub 실험에 일관되게 적용하였다. 동일한 데이터 구성 효과가 대규모 데이터에서도 재현됨을 확인하는 보강 근거로 사용한다.

### 3.2 데이터 분할 및 구성 선택

D-Fire는 공식 train/val/test 분할을 제공하므로, 검증셋(3,099장)과 테스트셋(4,306장)은 모든 실험에서 고정한다. 학습셋(14,122장) 안에서 ablation 구성별로 사용할 이미지를 선택하여 데이터 구성을 변화시킨다.

AIHub 대규모 재현 실험(§5.5)에서는 원천 영상에서 프레임을 직접 추출해야 하므로 3단계 씬 샘플링을 적용한다. 첫째, 씬 경계 탐지 단계에서는 히스토그램 상관계수가 0.7 이하로 떨어지는 지점을 씬 경계로 판정하며, 임계값 0.7은 씬 경계 누락과 과분할 사이의 균형을 최적화하는 값으로 예비 실험에서 선정하였다. 둘째, 씬 내 시간 간격 추출 단계에서는 씬 내에서 2초 간격(30fps 기준 60프레임당 1장)으로 프레임을 추출하며, 이 간격은 동일 씬 내 시각적 유사 프레임의 중복을 최소화하면서 장면 다양성을 확보하기에 충분한 것으로 예비 실험에서 확인하였다. 셋째, 지각 해시 중복 제거 단계에서는 지각적 해시(perceptual hash)의 해밍(Hamming) 거리가 8 이하인 프레임을 중복으로 제거하며, 임계값 8은 지각적으로 유사한 프레임을 효과적으로 제거하면서 미세한 장면 변화는 보존하는 값으로 imagehash 라이브러리 권장 범위(4~10) 내에서 선정하였다.
...
C2의 수량은 FL:SM 비율(약 14:1)을 통제 변수로 유지하면서 학습셋 내에서 결정된 근사치이다. C4의 NM 수량(6,458장)은 D-Fire 학습셋 내 정상 배경 이미지 전량을 사용한 결과이며, 이로 인해 FL:SM:NM 비율은 약 1:1:2.8이 된다. 정상 배경을 화재·연기보다 많게 구성한 것은 실제 감시 환경에서 화재 발생이 드물고 정상 장면이 대부분을 차지하는 현실을 반영한다. 평가용 검증셋·테스트셋은 전 구성에서 동일한 D-Fire 공식 분할(검증셋 3,099장, 테스트셋 4,306장)을 사용하여 직접 비교가 가능하도록 통제한다.
...
탐지기는 YOLO11 계열(n/s/m/l)[15]을 사용한다. 모든 학습은 COCO 사전학습 가중치에서 파인튜닝하며, 동일한 학습 설정을 적용한다: 옵티마이저 AdamW, 초기 학습률 0.001, 코사인 학습률 감쇠, warmup 3에폭, 최대 100에폭, patience 30(조기 종료), 데이터 증강(HSV 변형, 좌우 반전, mosaic 1.0, mixup 0.1). Ultralytics YOLO11 기본 설정을 베이스라인으로 사용하였으며, 초기 학습률 0.001은 사전학습 가중치 파인튜닝 시 원래 특징을 보존하면서 수렴 안정성을 높이기 위해 기본값보다 낮게 설정하였다. patience 30은 소규모 데이터셋에서 과적합 억제와 충분한 수렴 시간 확보의 균형을 위해 선택하였다.

### 4.2 실험 설계

두 개의 독립 ablation 실험을 수행한다. 실험 A(데이터 구성)에서는 모델을 YOLO11n으로 고정하고 데이터 구성을 C1→C2→C3→C4로 단계적으로 변화시켜 데이터 요인의 기여를 측정한다. 실험 B(모델 용량)에서는 데이터를 C4로 고정하고 모델을 YOLO11n→11s→11m→11l로 확대하여 용량 요인의 기여를 측정하며, YOLO11n·11s는 D-Fire 기준 주실험으로, YOLO11m·11l은 AIHub 175K 기준 보조 참고로 보고한다.

추가로 입력 해상도(640 vs 416)에 따른 정확도-속도 트레이드오프를 YOLO11n 기준으로 측정한다.

### 4.3 평가 지표

표준 객체 탐지 지표인 평균 정밀도(mean Average Precision, mAP)@0.5, mAP@0.5:0.95, 정밀도(precision), 재현율(recall), 클래스별 평균 정밀도(Average Precision, AP)를 사용한다.

비화재보 저감 관점에서 정밀도(precision)는 다음과 같이 정의된다.

$$\text{Precision} = \frac{TP}{TP + FP}$$

여기서 FP(false positive)는 정상 장면을 화재·연기로 오인한 탐지를 의미한다. 따라서 정밀도는 "탐지 결과 중 실제 화재·연기인 비율"로, 낮은 정밀도는 높은 비화재보 발생 가능성과 직결된다. 이에 본 연구에서는 정밀도를 비화재보 억제 능력의 대리 지표(proxy metric)로 활용하며, 실제 FP/FPR의 직접 측정은 향후 과제로 남긴다. 연기 탐지 능력은 smoke AP를 중점 분석한다.
...
구성별 주요 관찰을 서술한다. 화염 단독 구성(C1)은 연기를 전혀 탐지하지 못하였다. smoke AP가 0.000으로 연기만 존재하는 화재 초기 구간을 완전히 놓치며, fire AP는 0.650이나 smoke 클래스 탐지 실패로 인해 전체 mAP는 0.325에 그쳤다. 소량의 연기를 불균형하게 추가한 구성(C2, FL:SM=14:1)에서는 mAP가 0.455로 소폭 상승하고 smoke AP가 0.237로 처음 활성화되었으나, 이는 클래스 균형 이후(C3: 0.698)의 3분의 1 수준에 불과하여 절대량이 부족한 연기 표본으로는 smoke 탐지 능력이 임계점에 도달하지 못함을 보여준다. 클래스 균형을 달성한 구성(C3, FL:SM=1:1)에서는 smoke AP가 0.237에서 0.698로 도약하고 mAP가 0.691로 상승하였으며(C2 대비 +23.6%p), C1→C2→C3의 smoke AP 변화(0.000→0.237→0.698)가 비선형적임은 클래스 균형 도달 시 연기 탐지 능력이 임계점을 넘어 완전히 발현됨을 의미한다. 정상배경을 추가한 구성(C4)에서는 정상 장면을 NM 표본으로 포함함으로써 mAP가 0.736(C3 대비 +4.5%p)으로 상승하였고, 정밀도 0.746과 재현율 0.669가 함께 개선되어 오탐 억제와 탐지 누락 저감이 동시에 달성되었다.

그림 3은 각 구성의 혼동행렬 변화로 위 관찰을 시각적으로 확인하며, 그림 5는 smoke AP의 단계별 변화를 별도로 시각화한다.
...
![smoke AP 단계별 변화](G:\EXPERT_KCI_Paper\04_figures\dfire_4cell\fig_smoke_ap_trend_dfire.png)

**그림 5. 데이터 구성(C1→C4)에 따른 smoke AP 변화 (D-Fire 테스트셋, YOLO11n).** C1(0.000)→C2(0.237)→C3(0.698)의 비선형 급등은 클래스 균형 도달 시 smoke 탐지 능력이 임계점을 넘어 완전히 활성화됨을 보여준다.
...
> †³ YOLO11l은 AIHub 175K 데이터에서 100에폭 학습 3회 시도 모두 검증 손실이 발산하였다. 배치 크기(16→10) 재시도에서도 동일하게 발산이 반복되었으며, 데이터 규모 대비 과도한 모델 용량이 학습 불안정을 유발한 것으로 해석된다. 학습률 조정 등 추가 튜닝으로 수렴 가능성을 완전히 배제할 수 없으나, 동일 조건에서 하위 모델(11s, 11m)이 안정적으로 수렴한 점에서 모델 용량 자체의 불안정성으로 판단하였다.
...
모델을 YOLO11n에서 YOLO11s로 확대한 효과는 +1.3%p(0.736→0.749)에 그쳤다. AIHub 대규모 실험에서도 YOLO11m(0.914)이 YOLO11s(0.918)보다 낮아, 데이터 규모를 막론하고 모델 용량 증가의 수확 체감이 일관되게 나타났다. 최대 모델인 YOLO11l은 AIHub 175K 데이터에서 학습 손실이 반복적으로 발산하여 수렴에 실패하였다.
...
데이터 구성의 단계별 개선 중 가장 큰 단일 스텝(C2→C3 클래스 균형, +23.6%p)만을 모델 용량 확대의 단일 스텝(11n→11s, +1.3%p)과 비교하더라도 약 18배의 차이가 난다. 데이터 구성 전 구간의 누적 개선(C1→C4, +41.1%p)을 기준으로 하면 약 30배에 달하며, 비교 기준과 무관하게 두 요인의 기여도 차이가 일관됨을 확인할 수 있다. smoke AP 기준으로 C1→C2→C3의 변화(0.000→0.237→0.698)는 비선형적으로, 소량 불균형 연기 표본은 탐지를 부분적으로만 활성화하고 클래스 균형에 도달해야 비로소 연기 탐지 능력이 완전히 발현된다. 비화재보 저감을 위한 화재 탐지 성능은 모델 아키텍처가 아니라 데이터 구성이 지배하며, 특히 클래스 균형이 성능 임계점으로 작용한다. C3→C4 향상이 총량 증가가 아닌 NM 고유 효과에 기인함은 동일 총량 통제 실험(C4_eq 0.734 > C3_vol 0.720, §6.3)으로 별도 확인하였다.

### 5.4 입력 해상도 트레이드오프

주실험(D-Fire) 기준으로, YOLO11n에서 입력 해상도를 640에서 416으로 낮춘 결과 mAP는 0.736에서 0.700으로 −3.6%p 감소하였다. 반면 AIHub 대규모 데이터 기준에서는 0.911에서 약 0.915로 소폭 향상(+0.4%p)되어, 해상도 축소의 정확도 영향이 데이터셋 특성에 따라 상이함을 확인하였다. 해상도를 낮추면 엣지 추론 처리량이 약 1.6배 향상(imgsz=640: 235 FPS → 416: 375 FPS, §5.6)되므로, 정확도 손실 허용 여부는 배포 대상 데이터 분포에서 직접 측정하여 결정해야 한다.
...
C4 구성으로 학습한 YOLO11n을 Jetson Orin Nano(8GB)에서 jetson_clocks 적용 후 TensorRT FP16으로 변환하여 trtexec raw inference 기준으로 측정한 결과를 표 6에 정리한다.

**표 6. Jetson Orin Nano TRT FP16 raw inference 성능 (C4 YOLO11n)**

| imgsz | 처리량 (FPS) | 중앙값 지연 (ms) | DFire test mAP |
|-------|------------|----------------|---------------|
| 640 | 235 | 4.53 | 0.736 |
| 416 | 375 | 2.77 | 0.700 |

raw inference는 신경망 순전파만 측정한 수치로 전처리·NMS·후처리는 미포함된다. 일반 CCTV 입력(25~30 FPS)의 7배 이상에 해당하여, end-to-end 처리 오버헤드를 감안하더라도 다채널 실시간 교차검증에 충분한 처리 여력을 확보한다.

### 5.7 선행 연구와의 비교

본 연구의 최적 구성(C4) 결과를 선행 연구와 비교하여 맥락을 제시한다(표 5). 화재 탐지 분야에서는 연구마다 학습·평가 데이터셋이 상이하여 직접 수치 비교가 어려운 경우가 많다. 따라서 표 5는 비교 유형을 두 층위로 구분한다. 첫째, D-Fire 테스트셋을 동일하게 사용한 연구는 직접 비교 대상(●)으로, 둘째, 별도 데이터셋을 사용한 연구는 맥락 참고용(○)으로 표기한다. 본 연구의 테스트셋은 4,306장이며, 비교 대상 논문의 배포판에 따라 분할 수가 상이할 수 있음을 유의한다.
...
| Lv et al. (2025) [16] | YOLOv5s | D-Fire | 76.7% | 78.2% | 71.4% | ● 직접 |
| Lv et al. (2025) [16] | YOLOv8n | D-Fire | 75.4% | 79.3% | 69.6% | ● 직접 |
| Lv et al. (2025) [16] | CCi-YOLOv8n | D-Fire | 78.5% | 79.5% | 71.5% | ● 직접 |
...
> ● 직접: D-Fire 테스트셋 사용, 직접 수치 비교 유효. 본 연구 테스트셋 4,306장; 비교 논문의 배포판·분할 수는 상이할 수 있음.
> ●†: D-Fire 데이터 사용하나 80:20 자체 분할. 본 연구 테스트셋(4,306장)과 동일 여부 불명확, 참고 수준.
...
정상배경(NM) 추가가 가장 큰 개선을 가져온 점은 비화재보 저감 관점에서 직접적 함의를 갖는다. NM은 화재가 아닌 장면을 화재로 오인하지 않도록 학습시키는 배경 음성 표본으로 기능한다. C4에서 C3 대비 정밀도(0.698→0.746)와 재현율(0.654→0.669)이 함께 향상된 것은, 정상배경 학습이 탐지 민감도를 희생하지 않으면서 오탐을 억제함을 보여준다. 이는 영상 교차검증이 감지기 비화재보를 걸러내는 보조 수단으로 기능하기 위한 핵심 조건이다.

### 6.2 데이터 구성 우선 전략의 실무적 함의

본 결과는 엣지 화재 탐지 시스템 개발에서 자원 투입 우선순위를 시사한다. 더 큰 모델을 탐색하기보다, (1) 화염·연기 클래스를 균형 있게 확보하고 (2) 충분한 정상배경(NM)을 배경 음성 표본으로 포함하는 데이터 구성에 우선 투자하는 것이 비용 대비 효과가 크다. 특히 경량 모델(YOLO11n)로도 D-Fire 기준 0.736의 mAP를 달성하므로, 엣지 배포에서 모델 용량을 키울 실익은 제한적이다.

### 6.3 한계

본 연구에는 다섯 가지 한계가 있다. 첫째, 본 실험은 D-Fire 단일 벤치마크 기준이며 다른 공개 데이터셋에서의 추가 교차 검증은 향후 과제이다. AIHub 대규모 보강 실험(§5.5)은 동일한 경향을 지지하나, D-Fire는 테스트셋(4,306장), AIHub는 검증셋(19,080장)을 평가셋으로 사용하여 두 실험의 mAP 수치를 직접 비교하는 것은 유효하지 않다. 둘째, C2 결과는 데이터셋 의존성을 보인다. D-Fire에서 C2(0.455)는 C1(0.325)보다 높으나, AIHub 기존 실험(R2/R3)에서는 평가셋 불일치로 인해 C2(0.761) < C1(0.777) 역전 경향이 관찰된 바 있다. 이를 검증하기 위해 동일 검증셋 기준으로 AIHub C1·C2를 재실험한 결과(R9), C2(mAP 0.770) > C1(mAP 0.463)로 D-Fire와 동일한 방향이 확인되었으며 smoke AP도 C1(0.000)에서 C2(0.616)로 활성화되었다. 기존 R2/R3의 역전 경향은 평가셋 불일치에 기인한 것으로 판단된다. 다만 fire AP는 C1(0.927)이 C2(0.925)보다 미세하게 높아(−0.2%p), 소량의 연기 학습이 화염 탐지에 미치는 영향은 데이터셋에 따라 달라질 수 있음을 시사한다. 반면 smoke AP 기준의 비선형 패턴(부분 활성화 → 완전 부활)과 클래스 균형이 임계점이라는 결론은 두 데이터셋에서 일관된다. 셋째, C3→C4 구성 비교에는 교란 변수가 존재한다. D-Fire 기준 C4는 C3 대비 정상배경 6,458장 추가와 동시에 총 학습 데이터량이 4,598장에서 11,056장으로 증가하였다. 이를 검증하기 위해 동일 총량(7,664장) 조건에서 NM 추가(C4_eq: C3+NM 3,066장)와 신호 추가(C3_vol: C3+smoke 3,066장)를 직접 비교하는 통제 실험을 수행하였다. 그 결과 C4_eq(0.734)가 C3_vol(0.720)보다 +1.4%p 높아, C3→C4의 성능 향상이 총량 증가가 아닌 NM 고유 효과에 기인함이 확인되었다. 아울러 C4_eq(NM 3,066장, 0.734)와 C4(NM 6,458장, 0.736)의 차이가 +0.2%p에 그쳐 NM 효과는 약 3,000장 수준에서 포화에 가까운 양상을 보였다. 넷째, 비화재보 저감 효과는 정밀도·재현율로 간접 입증하였으며, 실제 현장 운영에서의 오경보 억제율 직접 측정은 별도 실증이 필요하다. 다섯째, 최대 모델(YOLO11l)의 학습 발산은 학습 설정 조정으로 일부 완화될 여지가 있으나, 본 데이터 규모에서 모델 용량 확대의 한계라는 결론에는 영향을 주지 않는다. 여섯째, 본 실험은 YOLO11 단일 계열에 국한되어 있어, 다른 아키텍처(RT-DETR, YOLO-NAS 등)에서 동일한 데이터 구성 효과가 재현되는지는 검증되지 않았으며 이는 향후 과제이다.

그림 6은 C4 구성 YOLO11n의 대표적인 탐지 성공·실패 사례를 제시한다.

![정성 탐지 예시 — 성공](G:\EXPERT_KCI_Paper\04_figures\dfire_4cell\fig_qualitative_success.png)

**그림 6a. 탐지 성공 사례 (C4 YOLO11n, D-Fire 테스트셋).** 야간 화재 장면에서 fire·smoke 복수 객체를 동시에 탐지한다(신뢰도 0.59~0.84).

![정성 탐지 예시 — 실패](G:\EXPERT_KCI_Paper\04_figures\dfire_4cell\fig_qualitative_failure.png)

**그림 6b. 탐지 실패 사례 (C4 YOLO11n, D-Fire 테스트셋).** 좌: 정상 산악 배경을 연기로 오탐(오탐지, false positive, FP; 신뢰도 0.25). 우: 이미지 대부분을 채운 짙은 연기를 미탐(미탐지, false negative, FN; 빨간 박스는 정답(ground truth, GT)).
...
[16] K. Lv, R. Wu, S. Chen, P. Lan, "CCi-YOLOv8n: Enhanced Fire Detection with CARAFE and Context-Guided Modules," in *Advanced Intelligent Computing Technology and Applications (ICIC 2025)*, Lecture Notes in Computer Science, vol. 15854, pp. 128–140, Springer, Singapore, 2025. doi:10.1007/978-981-96-9901-8_11.

[17] C. Li, Y. Du, X. Zhang, P. Wu, "YOLOGX: an improved forest fire detection algorithm based on YOLOv8," *Frontiers in Environmental Science*, vol. 12, Article 1486212, 2025. doi:10.3389/fenvs.2024.1486212.

[18] 소방청, "2022년 소방활동 빅데이터 분석 결과 발표," 보도자료, 2023. 2. 23. https://www.nfa.go.kr/nfa/news/pressrelease/press/?boardId=bbs_0000000000000010&mode=view&cntId=1733 (접속: 2026-06-25).
...
| C2 (AIHub) mAP / smoke AP | 0.761 / 0.684 | R3 — 평가셋 구성 불일치(R2와 상이), C1과 직접 비교 주의 |
| C1 (AIHub, R9) mAP / smoke AP / fire AP / P / R | 0.463 / 0.000 / 0.927 / 0.464 / 0.427 | R9 — val(19,080장) 일관 기준 |
| C2 (AIHub, R9) mAP / smoke AP / fire AP / P / R | 0.770 / 0.616 / 0.925 / 0.856 / 0.725 | R9 — val(19,080장) 일관 기준, C2>C1 확인 |
...
| 그림 | 파일 경로 | 커밋 |
|------|-----------|------|
| 그림 1 — 데이터 구성별 mAP | `04_figures/dfire_4cell/fig_data_composition_dfire.png` | c7a3b11 |
| 그림 2 — 모델 용량별 mAP | `04_figures/dfire_4cell/fig_model_dfire.png` | c7a3b11 |
| 그림 3 — 혼동행렬 2×2 grid | `04_figures/dfire_4cell/fig_confusion_matrix_comp_dfire.png` | 99d96a3 |
| 그림 4 — C4 PR 곡선 | `04_figures/dfire_4cell/C4_test_PR_curve.png` | 7daf568 |
| 그림 5 — smoke AP 트렌드 | `04_figures/dfire_4cell/fig_smoke_ap_trend_dfire.png` | c7a3b11 |
| 그림 6a — 탐지 성공 사례 | `04_figures/dfire_4cell/fig_qualitative_success.png` | 359e5d1 |
| 그림 6b — 탐지 실패 사례 | `04_figures/dfire_4cell/fig_qualitative_failure.png` | 359e5d1 |
